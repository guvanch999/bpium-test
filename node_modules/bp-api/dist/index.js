...
axios_1.default.interceptors.response.use((response) => response, (error) => {
    if (!BP.debug && error.isAxiosError && lodash_1.default.get(error, 'response.status', false)) {
        // console.log(error)
        const responseData = lodash_1.default.get(error, 'response.data', false);
        error.request = '-';
        error.response = {
...
class BP {
    ...
    constructor(domen, login, password, protocol = 'https', timeout = 30000) {
        ...
        this.sidCookie = '';
        if (!domen)
            throw new Error(`domen can't be empty`);
        if (!login)
    ...
    _getUrl(opt) {
        ...
        switch (opt.resource) {
            case 'record':
                return `${this.baseUrl}/catalogs/${opt.catalogId}/records/${(_a = opt.recordId) !== null && _a !== void 0 ? _a : ''}`;
            case 'catalog':
                return `${this.baseUrl}/catalogs/${(_b = opt.catalogId) !== null && _b !== void 0 ? _b : ''}`;
            case 'section':
                return `${this.baseUrl}/sections/${(_c = opt.sectionId) !== null && _c !== void 0 ? _c : ''}`;
            case 'view':
                return `${this.baseUrl}/catalogs/${opt.catalogId}/views/${(_d = opt.viewId) !== null && _d !== void 0 ? _d : ''}`;
            case 'board':
    ...
    async _requestWithAuthBasic(url, method, data = {}, params = {}) {
        ...
        const setCookies = lodash_1.default.get(result, 'headers["set-cookie"]', []);
    ...
    async _request(url, method, data = {}, params = {}) {
        if (!this.sidCookie) {
            ...
            try {
                ...
                const isAuthError = lodash_1.default.get(errorCookie, 'response.status', 0) == 401;
                if (!isAuthError) {
                    throw errorCookie;
                }
                return await this._requestWithAuthBasic(url, method, data, params);
    ...
    async getHistory(catalogId, recordId, params = {}) {
        ...
        const response = await this._request(url, 'GET', undefined, Object.assign({ catalogId, recordId }, params));
        return response.data;
    }
    /**
    ...
    async postCatalog(data) {
        ...
        if (lodash_1.default.isEmpty(data))
            throw new Error(`data cant't be empty`);
    ...
    async addCommentToHistory(catalogId, recordId, message) {
        ...
        if (typeof message !== 'string')
            throw new Error(`message need to be a string`);
        const url = this._getUrl({ resource: 'histories' });
    ...
    async getAllRecords(catalogId, params = {}, maxLimit = 5000) {
        ...
        if (!catalogId)
            throw new Error(`catalogId is required`);
        const initOffset = (_a = params.offset) !== null && _a !== void 0 ? _a : 0;
        const initChunkLimit = (_b = params.limit) !== null && _b !== void 0 ? _b : 500;
        const totalRecords = [];
        while (totalRecords.length < maxLimit) {
            ...
            const records = await this.getRecords(catalogId, Object.assign(Object.assign({}, params), { limit: chunkLimit, offset: initOffset + totalRecords.length }));
     ...
     * ```
     */
    async uploadFile(fileKeys, streamOrBuffer) {
        if (!streamOrBuffer)
            throw new Error(`readble stream or buffer is required`);
        if (!fileKeys)
            throw new Error(`fileKeys is required. First use method getUploadFileKeys`);
        ...
        const fileLength = await new Promise((resolve, reject) => {
        ...
        try {
            await axios_1.default.request({
                ...
                data: formData,
                headers: Object.assign(Object.assign({}, formHeaders), { 'Content-Length': fileLength }),
